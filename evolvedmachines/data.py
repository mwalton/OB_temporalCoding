...
class olfactoryDataset:
    ...
    def loadTraining(self, folder):
        reader = csv.reader(open(folder + "concentration.csv","rb"), delimiter=",")
        x = list(reader)
        self.train_c = np.array(x).astype('float')
        
        reader = csv.reader(open(folder + "sensorActivation.csv","rb"), delimiter=",")
        x = list(reader)
